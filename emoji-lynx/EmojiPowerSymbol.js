import _attrs from '../default_attrs.js'
import { lynxIcon } from '../lynx_svg.js'

/** Mithril-lynx component for the OpenMoji "power symbol" (color) [23FB]. */
const EmojiPowerSymbol = lynxIcon(`
  <g id="color">
    <circle cx="35.9805" cy="35.9146" r="19.0845" fill="#fff"/>
    <path fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m29.5272,16.917s5.7317-2.3936,13.128.0718"/>
  </g>
  <g id="line">
    <path fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m29.3333,17c-7.7679,2.7553-13.3333,10.1899-13.3333,18.929,0,11.0849,8.9543,20.071,20,20.071s20-8.9861,20-20.071c0-8.7391-5.5654-16.1737-13.3333-18.929"/>
    <path fill="none" stroke="#000" stroke-linecap="round" stroke-width="2" d="m36,11v25"/>
  </g>
`, _attrs, 72)
export default EmojiPowerSymbol
